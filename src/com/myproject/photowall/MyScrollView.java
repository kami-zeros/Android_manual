...
public class MyScrollView extends ScrollView implements OnTouchListener {
	...
	public static final int PAGE_SIZE=14;//每页要加载的图片数量 
...
//	 */
	...
	public void loadMoreImages() {
		...
		new Thread() {
			public void run() {
				try {
					imglist = Images.getImgUrl();
//					Log.d("TAG", "每张图片:"+imglist);
				} catch (Exception e) {
					e.printStackTrace();
				}
			};
...
//		Log.d("TAG", "每张图片:"+Images.imgUrllist.size());
		if (hasSDCard()) {
			int startIndex = page * PAGE_SIZE;
			int endIndex = page * PAGE_SIZE + PAGE_SIZE;
			if (startIndex < imglist.size()) {	//Images.imageUrls.length

				Toast.makeText(getContext(), "正在加载...", Toast.LENGTH_SHORT)
						.show();
				if (endIndex > imglist.size()) {  //Images.imageUrls.length
					endIndex = imglist.size();
				}
				for (int i = startIndex; i < endIndex; i++) {
					LoadImageTask task = new LoadImageTask();
					taskCollection.add(task);
					task.execute(imglist.get(i));//Images.imageUrls[i]
				}
				page++;
			} else {
